...
@end
typedef void(^pickerBlock)(NSInteger item);

@interface AKPickerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate,AKPickerViewDataSource, AKPickerViewDelegate>
...
+(AKPickerView *)pickerViewWithSupView:(UIView *)supView frame:(CGRect)frame dataArray:(NSArray *)dataArray block:(void(^)(NSInteger item))block{
   AKPickerView * pickerView = [[AKPickerView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, 50)];
...
-(void)setBlock:(pickerBlock)block{
    if (block) {
        _block = block;
    }
}
